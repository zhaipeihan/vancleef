...
/**
 * Converts between an array of bytes and a Base58Util string. Not instantiable.
 */
public class Base58Util {
    ...
    // Adds the checksum and converts to Base58Util. Note that the caller needs to prepend the version byte(s).
    public static String bytesToBase58(byte[] data) {
        return bytesToBase58WithoutCheckHash(addCheckHash(data));
    }


    // Converts the given Base58Util string to a byte array, verifies the checksum, and removes the checksum to return the payload.
    ...
    public static byte[] base58ToBytes(String s) {
        ...
        byte[] rehash = Arrays.copyOf(HashUtil.doubleHash(data), 4);
    ...
    // Directly converts to Base58Util without adding a checksum.
    private static String bytesToBase58WithoutCheckHash(byte[] data) {
    ...
    private static byte[] addCheckHash(byte[] data) {
        try {
            byte[] hash = Arrays.copyOf(HashUtil.doubleHash(data), 4);
    ...
    // Converts the given Base58Util string to a byte array, without checking or removing the trailing 4-byte checksum.
    private static byte[] base58ToRawBytes(String s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            ...
            if (digit == -1)
                throw new IllegalArgumentException("Invalid character for Base58Util");
